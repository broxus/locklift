      ...
      await from(contractsTree)
        .pipe(
          ...
          mergeMap(({ path, contractFileName }) => {
            ...
            const additionalIncludesPath = `--include-path ${resolve(
              process.cwd(),
              "node_modules"
          ...
          mergeMap(({ contractFileName }) => {
            const lib = this.config.linkerLibPath
              ? ` --lib ${this.config.linkerLibPath} `
              : "";
            const resolvedPathCode = resolve(
              this.options.build,
              `${contractFileName}.code`
            );
            const resolvedPathAbi = resolve(
              this.options.build,
              `${contractFileName}.abi.json`
            );
            return defer(async () =>
              promisify(exec)(
                `${this.config.linkerPath} compile "${resolvedPathCode}" -a "${resolvedPathAbi}" ${lib}`
              )
            ).pipe(
              map((tvmLinkerLog) => {
                return tvmLinkerLog.stdout
                  .toString()
              ...
              mergeMap((tvcFile) => {
                return concat(
                  ...
                  defer(() =>
                    promisify(fs.rename)(
                      tvcFile,
                      resolve(this.options.build, `${contractFileName}.tvc`)
                    )
                  )
                ).pipe(
                  catchError((e) => {
