...
export const compilerConfigResolver = async ({
  ...
  if ("version" in linker) {
    if (!("version" in compiler)) {
      throw new Error(
        "You can't provide linker version without compiler version!"
      );
    }
...
export function execSyncWrapper(
  ...
  try {
    ...
    throw new Error(
      `${ioError.toString()}stdout: ${ioError.stdout.toString()}`
    );
  }
}
...
export const isValidCompilerOutputLog = (output: string): boolean =>
  !!output.trim() &&
  output.trim() !== "Compiler run successful, no output requested.";
